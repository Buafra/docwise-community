...
1. **Tesseract** (always, local) — best-accuracy Arabic/English models, ~100-pass search per page with confidence gating
2. **Ollama vision model** (optional, local, free) — auto-detected; rescues ID cards, stamps, handwriting, busy backgrounds
3. **Azure Document Intelligence** (optional, paid ~$0.0015/page) — set `AZURE_DI_ENDPOINT` and `AZURE_DI_KEY`; used only when local confidence is low
4. **OpenAI Vision** (optional, paid) — set `OPENAI_API_KEY`; last resort for the hardest pages

Only Tesseract is required — the app works fully with nothing else configured. Force a specific engine with `DOCWISE_OCR` in `.env` (`tesseract` / `ollama` / `azure` / `openai`).
